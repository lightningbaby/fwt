...
class MatchingNet(MetaTemplate):
  ...
  def set_forward(self, x, is_feature = False): # [5,21,3,224,224]
    z_support, z_query  = self.parse_feature(x,is_feature)# [5,5,512] [5,16,512] resnet extract features

    z_support   = z_support.contiguous().view( self.n_way* self.n_support, -1 )# [25,512]
    z_query     = z_query.contiguous().view( self.n_way* self.n_query, -1 )# [80,512]
    G, G_normalized = self.encode_training_set( z_support)# 都是【25，512】

    y_s         = torch.from_numpy(np.repeat(range( self.n_way ), self.n_support ))# 【25】,0-4
    Y_S = utils.one_hot(y_s, self.n_way)# [25,5] one-hot label
    if torch.cuda.is_available():
      Y_S         = Y_S.cuda()
    f           = z_query
    logprobs = self.get_logprobs(f, G, G_normalized, Y_S)# 【80，5】,fully contextrual embedding
    return logprobs

  def set_forward_loss(self, x):# x[5,21,3,224,224] [n-way,n_support5+n_query16,channel,h,w]
    y_query = torch.from_numpy(np.repeat(range( self.n_way ), self.n_query ))# [80]
    if torch.cuda.is_available():
      y_query = y_query.cuda()

    logprobs = self.set_forward(x)# 【80，5】 resnet extract features(def parse features) ,then fce
    loss = self.loss_fn(logprobs, y_query)# nllloss
